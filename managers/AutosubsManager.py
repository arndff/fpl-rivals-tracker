...
class AutosubsManager(Manager):
    ...
    def __init_autosubs(self):
        autosubs_ids = self.event_data_parser.get_autosubs()

        if len(autosubs_ids) == 0:
            self.__players_out = self.__players_in = "None"
            self.__total_bench_points = self.team_data_parser.get_bench_points_in_specific_gw(self.current_event)
        ...
        for player_out_id in autosubs_ids:
            player_out = self.event_data_parser.get_player_name(player_out_id)

            player_in_id = autosubs_ids[player_out_id]
            player_in = self.event_data_parser.get_player_name(player_in_id)
        ...
        self.__total_bench_points = self.__used_bench_points + \
                                    self.team_data_parser.get_bench_points_in_specific_gw(self.current_event)
...
class Player:
    ...
    def to_list(self):
        result = [self.name, self.points, self.count]
        return result
